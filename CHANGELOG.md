...
### Unreleased
- **2026-07-08**: Updated the About page bio to reference Zozo, City Drive, and Turf Wars as plain inline links to their project pages, mirroring the existing Moodful mention (no new card/tag markup or shared data module).
...
- **2026-03-07**: Updated chat UX: message timestamps now show full date + time, removed the non-working Option/Alt shortcut actions from both UI and key handlers, and made the Identity panel collapsible with persisted default-collapsed behavior after profile selection while keeping avatar + current name visible in collapsed mode on a single horizontal row.
