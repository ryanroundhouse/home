 ...
 *
 * @param {{ name?: string, svg?: string, onClose?: () => void, immediate?: boolean }} options
 *   `immediate` skips the crank/drop/crack cinematic and opens straight to the
 *   final close-up reveal — used when re-viewing an already-owned capsule.
 * @returns {{ close: () => void }}
 */
export function openGashaponCapsuleModal({ name, svg, onClose, immediate = false } = {}) {
  ...
  if (prefersReducedMotion || immediate) {
    ...
    // `immediate` re-views an owned capsule; reduced-motion honors the OS pref.
    setPhase('reveal');
  } else {
    playSequence();
  }
